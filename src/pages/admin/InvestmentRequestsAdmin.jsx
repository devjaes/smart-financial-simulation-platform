...
export default function InvestmentRequestsAdmin() {
  const [requests, setRequests] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    api.getInvestmentRequests(500).then((data) => {
  ...
  const metrics = useMemo(() => ({
    ...
    capitalTotal: requests.reduce((s, r) => s + r.capitalInicial, 0),
  }), [requests])
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Solicitudes de inversión</h1>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          ...
          { label: 'Capital total', value: formatCurrency(metrics.capitalTotal), color: '#64748b' },
        ].map((m) => (
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm ring-1 ring-slate-900/5">
        <table className="min-w-full text-left text-xs">
          <thead className="sticky top-0 z-10 border-b border-slate-200/70 bg-linear-to-b from-white via-slate-50/90 to-slate-50 text-slate-500 backdrop-blur">
            <tr>
            ...
            {requests.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-semibold tabular-nums text-slate-800">{formatCurrency(r.capitalInicial)}</td>
                  ...
                  <td className="px-3 py-3 text-right">
                    <Link
                      to={`/admin/solicitudes-inversion/${r.id}`}
                      className={[
