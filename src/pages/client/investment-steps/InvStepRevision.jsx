...
export default function InvStepRevision() {
  ...
  const refreshStatus = async () => {
    ...
    setRefreshing(true)
    try {
      const data = await api.getInvestmentRequest(requestId)
      if (data) {
        updateFields({ estado: data.estado, notasAsesor: data.notasAsesor || '' })
        if (['Aprobado', 'Rechazado'].includes(data.estado)) setStep(9)
      }
    } catch { /* ignore */ }
    setRefreshing(false)
  }
  ...
  return (
    <div className="space-y-5 py-8 text-center">
      ...
      <h2 className="text-xl font-semibold text-slate-900">Solicitud de inversión en revisión</h2>
      ...
      <div className="mx-auto max-w-sm rounded-xl bg-slate-50 p-4 text-left text-sm">
        ...
        <div><strong>Capital:</strong> {formatCurrency(ctx.capitalInicial)}</div>
