...
export default function InvStepRegistro() {
  const { user } = useAuth()
  const { updateField, updateFields, setStep } = useInvestmentRequest()
  ...
  const handleNext = async () => {
    ...
    setChecking(true)
    try {
      // Pre-fill investment request context with user data
      updateFields({
        cedula: user.cedula,
        email: user.email,
      ...
      // Check for existing active investment requests
      const existing = await api.getInvestmentRequestsByCedula(user.cedula)
      const active = existing.find((r) => !['Aprobado', 'Rechazado'].includes(r.estado))
      if (active) {
        const goToStatus = confirm(
          `Ya tiene una solicitud de inversión activa (${active.estado}). ¿Desea ver su estado?`,
        )
        if (goToStatus) {
          updateFields({
            ...
            capitalInicial: active.capitalInicial,
          })
          setStep(8)
          return
        }
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">Registro del inversionista</h2>
        <p className="mt-1 text-sm text-slate-600">
          Ingrese sus datos de acceso para iniciar la solicitud de inversión.
        </p>
      </div>
