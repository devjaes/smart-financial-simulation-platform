...
export default function InvStepResultado() {
  ...
  useEffect(() => {
    if (!requestId) return
    api.getInvestmentRequest(requestId).then((data) => {
  ...
  return (
    <div className="space-y-5 py-8 text-center">
      ...
      <h2 className="text-xl font-semibold" style={{ color: isApproved ? '#166534' : '#991b1b' }}>
        Solicitud de inversión {isApproved ? 'aprobada' : 'rechazada'}
      </h2>
      <p className="mx-auto max-w-md text-sm text-slate-600">
        {isApproved
          ? 'Su inversión ha sido aprobada. Un asesor se comunicará con usted para los siguientes pasos.'
          : 'Lamentablemente su solicitud de inversión no fue aprobada en esta ocasión.'}
      </p>
