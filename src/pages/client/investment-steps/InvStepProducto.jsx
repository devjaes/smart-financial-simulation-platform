...
export default function InvStepProducto() {
  const { productoId, updateFields, setStep } = useInvestmentRequest()
  ...
  const handleSelect = (p) => {
    updateFields({
      ...
      capitalizacion: p.capitalizacion || 'mensual',
    })
  }
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Elija el producto de inversión que desea solicitar.</p>
      </div>
        ...
        {investmentProducts.map((p) => (
          <button
            ...
            className={[
              ...
              productoId === p.id ? 'border-emerald-500 bg-emerald-50/50 shadow' : 'border-slate-200 bg-white',
            ].join(' ')}
          >
            ...
            <div className="mt-3 space-y-1 text-xs text-slate-500">
              <p>Mínimo: {formatCurrency(p.montoMin)}</p>
              <p>Capitalización: {p.capitalizacion}</p>
              <p>Plazos: {(p.plazoMeses || []).join(', ')} meses</p>
            </div>
          </button>
        ))}
      </div>
