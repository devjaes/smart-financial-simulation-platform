...
export default function ClientInvestmentRequests() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        const data = await api.getInvestmentRequestsByCedula(user.cedula)
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">Mis inversiones</h1>
        <p className="mt-1 text-sm text-slate-600">
          Consulte el estado de sus solicitudes de inversión.
        </p>
      </div>
